...
class UI {
  ...
  getBagButtons() {
    ...
    buttons.forEach((button) => {
      ...
      if (inCart) {
        (button.innerText = "En Carrito"), (button.disabled = true);
      }
  ...
  setCartValues(cart) {
    ...
    totalCarrito.innerText = parseFloat(tempTotal.toFixed(2));
  ...
  setupAPP() {
    ...
    cerrarCarrito.addEventListener("click", this.hideCart);
  }
  populateCart(cart) {
  ...
  cartLogic() {
    ...
    contenidoCarrito.addEventListener("click", (event) => {
      if (event.target.classList.contains("remove-item")) {
        ...
        let sumaPrecio = event.target;
        let id = sumaPrecio.dataset.id;
        let productoTemp = cart.find((item) => item.id === id);
        productoTemp.amount = productoTemp.amount + 1;
        Storage.saveCart(cart);
        this.setCartValues(cart);
        sumaPrecio.nextElementSibling.innerText = productoTemp.amount;
      } else if (event.target.classList.contains("fa-chevron-down")) {
        let restaPrecio = event.target;
        let id = restaPrecio.dataset.id;
        let productoTemp = cart.find((item) => item.id === id);
        productoTemp.amount = productoTemp.amount - 1;
        if (productoTemp.amount > 0) {
          Storage.saveCart(cart);
          this.setCartValues(cart);
          restaPrecio.previousElementSibling.innerText = productoTemp.amount;
        } else {
          contenidoCarrito.removeChild(restaPrecio.parentElement.parentElement);
          this.removeItem(id);
        }
      }
    });
  ...
  clearCart() {
    ...
    console.log(contenidoCarrito.children);
